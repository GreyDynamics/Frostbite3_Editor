							...
							case 0: //add base blocks without modification
								for(int i=0; i<instructionSize;i++){
					                ...
					                if (entry.getCurrentSize()==entry.getOriginalSize()){
					                    entry=next(entriesIndex, baseSeeker, deltaSeeker, baseOffset, deltaOffset);//updateSize of current Entry and go to next!
...
//		}
	...
	private NonCasBundleEntry next(int entriesIndex, FileSeeker baseSeeker, FileSeeker deltaSeeker, int baseBundleOffset, int deltaBundleOffset){
		updateEntrySize(entriesIndex, baseSeeker, deltaSeeker, baseBundleOffset, deltaBundleOffset);
		entriesIndex++;
		return updateEntryOffset(entriesIndex, baseSeeker, deltaSeeker, baseBundleOffset, deltaBundleOffset);
	}
	
	private NonCasBundleEntry updateEntryOffset(int entriesIndex, FileSeeker baseSeeker, FileSeeker deltaSeeker, int baseBundleOffset, int deltaBundleOffset){
		if (entriesIndex<this.entries.size()){
			NonCasBundleEntry entry = this.entries.get(entriesIndex);
			entry.setBaseOffset(baseSeeker.getOffset()+baseBundleOffset);
			entry.setDeltaOffset(deltaSeeker.getOffset()+deltaBundleOffset);			
			return entry;
		}
		return null;
	}
	
	private NonCasBundleEntry updateEntrySize(int entriesIndex, FileSeeker baseSeeker, FileSeeker deltaSeeker, int baseBundleOffset, int deltaBundleOffset){
		if (entriesIndex<this.entries.size()){
			NonCasBundleEntry entry = this.entries.get(entriesIndex);
			entry.setBaseSize(baseSeeker.getOffset()-(entry.getBaseOffset()+baseBundleOffset));
			entry.setDeltaSize(deltaSeeker.getOffset()-(entry.getDeltaOffset()+deltaBundleOffset));
			return entry;
		}
		return null;
	}
